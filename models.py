...
class EnhancedKPipeline(KPipeline):
    ...
    def __init__(self, lang_code: str = 'a', model: bool = True):
        try:
            # Try to initialize the base pipeline as usual
            super().__init__(lang_code=lang_code, model=model)
        except RuntimeError as e:
            # Detect MeCab / fugashi initialization failures and attempt a best-effort fix
            err_str = str(e)
            if ('Failed initializing MeCab' in err_str) or ('mecabrc' in err_str) or ('fugashi' in err_str) or ('no such file or directory' in err_str):
                logger.warning("MeCab initialization failed during pipeline init: %s", err_str)
                logger.info("Attempting to automatically install/download Unidic/Fugashi resources and retry initialization.")
                try:
                    _try_fix_mecab()
                    # Retry once
                    super().__init__(lang_code=lang_code, model=model)
                    ...
                    help_msg = (
                        "Failed to initialize MeCab / fugashi automatically.\n"
                        "To fix manually, run:\n"
                        "  pip install -U 'fugashi[unidic-lite]' unidic-lite\n"
                        "  python -m unidic download\n"
                        "Or install system MeCab and a dictionary (macOS/Homebrew example):\n"
                        "  brew install mecab mecab-ipadic\n"
                        "Then ensure mecab is available in your PATH and try again.\n"
                        "See https://github.com/polm/fugashi for details."
                    )
                    logger.error(help_msg)
                    raise RuntimeError(help_msg) from fix_exc
            else:
...
def _try_fix_mecab() -> None:
    """Attempt to install unidic-lite and download dictionaries so fugashi/MeCab can initialize.

    This is a best-effort helper that will attempt to pip-install missing packages and run
    ``python -m unidic download``. It will then attempt to instantiate a fugashi Tagger to
    verify success. This may require network connectivity and appropriate permissions.
    """
    try:
        # Ensure fugashi is installed
        try:
            import fugashi  # noqa: F401
            logger.info("fugashi already installed")
        except Exception:
            logger.info("Installing fugashi[unidic-lite] and unidic-lite via pip...")
            subprocess.check_call([sys.executable, "-m", "pip", "install", "--upgrade", "fugashi[unidic-lite]", "unidic-lite"])

        # Attempt to download the unidic dictionary
        try:
            logger.info("Downloading unidic dictionary (this may take a while)...")
            subprocess.check_call([sys.executable, "-m", "unidic", "download"])
        except subprocess.CalledProcessError as e:
            logger.warning("`python -m unidic download` failed: %s", e)

        # Test by creating a Fugashi Tagger
        from fugashi import Tagger
        Tagger()
        logger.info("fugashi Tagger initialized successfully after attempted fix.")

    except Exception as exc:
        logger.warning("Auto-install attempt failed: %s", exc)
        raise
